...
def validate_credits_excel(excel_path: str, valid_title_numbers: List[str]) -> Tuple[bool, List[str], Optional[pd.DataFrame]]:
    ...
    if len(invalid_titles) > 0:
        ...
        if len(invalid_list) > 10:
            errors.append(f"Title numbers not found in dataset ({len(invalid_list)} total): {invalid_list[:10]}...")
        else:
    ...
    for idx, val in enumerate(df['Amount']):
        try:
            ...
            if val_clean in ['', 'nan', 'None']:
                errors.append(f"Row {idx + 2}: Empty amount")
                amounts.append(None)
            else:
            ...
            errors.append(f"Row {idx + 2}: Invalid amount format '{val}'")
            amounts.append(None)
    ...
    for idx, val in enumerate(df['Start Date']):
        try:
            ...
            if val_str in ['', 'nan', 'None']:
                errors.append(f"Row {idx + 2}: Empty start date")
                dates.append(None)
            else:
                parsed_date = None
                ...
                if parsed_date is None:
                    try:
                        parsed_date = pd.to_datetime(val_str, errors='raise').to_pydatetime()
                    except:
                        errors.append(f"Row {idx + 2}: Invalid date format '{val}'")
                        dates.append(None)
                        continue
...
def calculate_credit_expiration_date(start_date: datetime, years_validity: int = 12) -> datetime:
    """
    Calculate credit expiration date based on Quebec Mining Act Section 75.
    Credits can be applied to six subsequent terms (6 x 2 years = 12 years).
    """
...
def load_and_apply_credits(excel_path: str, gdf: 'gpd.GeoDataFrame', output_dir: Optional[str] = None) -> Tuple['gpd.GeoDataFrame', int, float, List[str]]:
    ...
    if not is_valid:
        ...
        for error in errors:
            print(f"  ERROR: {error}")
        raise ValueError("Credits Excel file validation failed.")
    ...
    for _, row in credits_df.iterrows():
        ...
        for idx in matching_indices:
            new_credit = {'date': pd.Timestamp(expiration_date), 'amount': round(amount, 2)}
            ...
            credits_applied += 1
            total_amount += amount
            credited_title_numbers.add(gdf.at[idx, 'title_no'])
